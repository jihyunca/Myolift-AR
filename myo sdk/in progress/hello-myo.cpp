...
public:
	...
	// Be warned: This will not make any distiction between data from other Myo armbands
	double onGyroscopeData(myo::Myo *myo, uint64_t timestamp, const myo::Vector3< float > &gyro, double y) {
		double old_y = printVector(gyroFile, timestamp, gyro, y);
		return old_y;
	}
	...
	double printVector(std::ofstream &file, uint64_t timestamp, const myo::Vector3< float > &vector, double y) {
		using std::cout;
		using std::endl;

		double new_y = vector.y();

		cout << "Testing";
		//Biceps
		double displacement_y = vector.y() - y;
		if ((displacement_y) < 0)
			displacement_y = displacement_y * -1;

		if ((displacement_y) > 10)
			cout << "INCORRECT POSITIONING. ADJUST SIMILAR TO THE COACH SHOWN" << endl;

		file << timestamp << ',' << vector.x() << ',' << vector.y() << ',' << vector.z() << std::endl;
		return new_y;
	}
